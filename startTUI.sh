...
[[ -x "$SCRIPT_DIR/rs41_tui.pl" ]] \
	|| fail "A rs41_tui.pl nem található vagy nem futtatható."

perl -MCurses -e '1' 2>/dev/null \
	|| fail "A Perl Curses modul nincs telepítve. Debian/Ubuntu: sudo apt install libcurses-perl"
...
frontend_path = os.path.join(base_dir, "rs41_tui.pl")
...
try:
	...
	terminal_line("startTUI", "ERR", str(error))
	sys.exit(2)
...
for name, value in sorted(config_overrides.items()):
	terminal_line("startTUI", "INFO", f"Config override: {name}={value}")
...
threads = [
	...
	threading.Thread(
		target=json_relay,
		args=("rs41_tui", frontend_process.stdout, main_process.stdin, False),
		daemon=True,
	),
	threading.Thread(
		target=stderr_relay,
	...
	threading.Thread(
		target=stderr_relay,
		args=("rs41_tui", frontend_process.stderr),
		daemon=True,
	),
]
...
terminal_line(
	"startTUI",
	"INFO",
	"Az rs41_main.pl és rs41_tui.pl folyamatok elindultak, a kétirányú JSON-csatornák összekapcsolva.",
)
	...
	while True:
		...
		if main_code is not None or frontend_code is not None:
			...
			if frontend_code not in (None, 0):
				terminal_line("rs41_tui", "ERR", f"A folyamat kilépési kódja: {frontend_code}")
...
terminal_line("startTUI", "INFO", "Mindkét folyamat leállt.")
sys.exit(exit_code)
PYCODE
